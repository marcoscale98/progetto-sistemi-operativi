...
int main(int argc,char *argv[]){
	...
	close_group = chiudo_gruppo(&is_leader, hanno_risp);

	if (aula->time_left >= closing_time && n_invitati<nof_invites &&  !accettato_invito && !close_group) {
...
int rispondo_inviti(int *accettato, int *is_leader, int *n_rifiutati, int max_reject, int *inviti, int hanno_risp) {
	...
	if(inviti[matricola]==INVITATO){
	   inviti[matricola]=LIBERO;
	    ...
	    //se il gruppo di chi mi invita è chiuso, rifiuto
	    else if(aula->group[matricola].is_closed) {
		rifiuta_invito(matricola);
		*n_rifiutati +=1;
	    }
...
int chiudo_gruppo(int *is_leader, int hanno_risp) {
    ...
    //se non tutti hanno risposto al mio invito non posso chiudere il gruppo
    else if(!hanno_risp);
